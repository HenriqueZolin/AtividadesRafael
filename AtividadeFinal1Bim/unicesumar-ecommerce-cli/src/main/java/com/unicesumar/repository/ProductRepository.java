...
public class ProductRepository implements EntityRepository<Product> {
    ...
    public List<Optional<Product>> findMultipleIds(String ids){
        List<Optional<Product>> listaNaoFormatada = new ArrayList<>();
        String[] listaIds;
        listaIds = ids.split(",");

        for(String id : listaIds){
            listaNaoFormatada.add(this.findById(UUID.fromString(id)));
        }

        return listaNaoFormatada;
